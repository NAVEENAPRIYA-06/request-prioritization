...
const SLATracker = () => {
  ...
  const getStatusConfig = (ms) => {
    if (ms < 0) return { label: 'Overdue', color: 'text-rose-500', bg: 'bg-rose-50', border: 'border-rose-500', icon: <AlertTriangle size={20} /> };
    if (ms < 3600000) return { label: 'Near Deadline', color: 'text-amber-500', bg: 'bg-amber-50', border: 'border-amber-400', icon: <Clock size={20} /> };
    return { label: 'On Track', color: 'text-emerald-500', bg: 'bg-emerald-50', border: 'border-emerald-400', icon: <Timer size={20} /> };
  };
    ...
    <div className="p-12 animate-in fade-in slide-in-from-bottom-4 duration-1000 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-1 gap-6">
        {requests.length > 0 ? requests.map((req) => {
          ...
          return (
            <div key={req.id} className={`bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/40 border-l-8 transition-all hover:scale-[1.01] flex items-center justify-between ${config.border}`}>
              <div className="flex items-center space-x-8">
                ...
                <div>
                  <div className="flex items-center space-x-3 mb-1">
                    ...
                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md ${req.priority === 'Critical' ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-500'}`}>
                      {req.priority}
                    </span>
                  </div>
                  <div className="flex items-center space-x-4">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center">
                      <Zap size={10} className="mr-1" /> {req.status}
                    </span>
                    {req.autoEscalation === 'Active' && (
                      <span className="text-[9px] font-black text-rose-500 uppercase bg-rose-50 px-2 py-0.5 rounded-lg border border-rose-100 animate-pulse">
                        Auto-Escalated
                      </span>
                    )}
                  </div>
              ...
              <div className="text-right">
                <div className={`text-3xl font-black tracking-tighter italic mb-1 ${config.color}`}>
